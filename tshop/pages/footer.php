        <!-- *** FOOTER ***
 _________________________________________________________ -->
        <div id="footer" data-animate="fadeInUp">
            <div class="container">
                <div class="row">

                    <div class="col-md-12 col-sm-12 text-center" >

                        <h4>Alamat</h4>

                        <p><strong>Henpon</strong>
                            <br>Jawa Barat
                            <br>Bogor
                            <br>Jl.Naik naik Ke puncak gunung
                            <br>tinggi tinggi Sekali 16721
                            <br>Depan kantor potong bebek angsa
                        </p>

                    </div>
                    <!-- /.col-md-3 -->

                </div>
                <!-- /.row -->

            </div>
            <!-- /.container -->
        </div>
        <!-- /#footer -->

        <!-- *** FOOTER END *** -->




        <!-- *** COPYRIGHT ***
 _________________________________________________________ -->
        <div id="copyright">
            <div class="container">
                <div class="col-md-6">
                    <p class="pull-left">© 2015 Your name goes here.</p>

                </div>
                <div class="col-md-6">
                    <p class="pull-right">Template by <a href="https://bootstrapious.com/e-commerce-templates">Bootstrapious.com</a>
                         <!-- Not removing these links is part of the license conditions of the template. Thanks for understanding :) If you want to use the template without the attribution links, you can do so after supporting further themes development at https://bootstrapious.com/donate  -->
                    </p>
                </div>
            </div>
        </div>
        <!-- *** COPYRIGHT END *** -->



    </div>
    <!-- /#all -->


    

    <!-- *** SCRIPTS TO INCLUDE ***
 _________________________________________________________ -->
    <script src="views/js/jquery-1.11.0.min.js"></script>
    <script src="views/js/bootstrap.min.js"></script>
    <script src="views/js/jquery.cookie.js"></script>
    <script src="views/js/waypoints.min.js"></script>
    <script src="views/js/modernizr.js"></script>
    <script src="views/js/bootstrap-hover-dropdown.js"></script>
    <script src="views/js/owl.carousel.min.js"></script>
    <script src="views/js/front.js"></script>

<script>
    navigator.serviceWorker.register('./sw.js')
    .then(function() {
      console.log('registered service worker')
    }).catch(function() {
      console.log('error')
    })
</script>

</body>

</html>
